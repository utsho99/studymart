...
router.post('/forgot-password', async (req, res) => {
  ...
  // For now return the code directly (no email service)
  // In production you'd send via email
  res.json({ message: 'Reset code generated', code, email });
});

// POST /api/auth/reset-password - use code to reset
router.post('/reset-password', async (req, res) => {
  ...
  if (new Date() > user.resetCodeExpiry) return res.status(400).json({ message: 'Reset code expired. Please try again.' });
